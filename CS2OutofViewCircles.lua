local ref = gui.Reference("Visuals", "World", "Helper")
local OOV_always = gui.Checkbox(ref, "OOV.always", "Consider FOV", false)
local OOV_color = gui.ColorPicker(ref, "OOV.clr", "Out Of Viev Color", 255, 255, 255, 255)
...
local function Draw()
	...
	local CCSPlayer = entities.FindByClass("C_CSPlayerPawn")
	for k, v in pairs(CCSPlayer) do
		...
		if players.activity[index] then
			alpha[index] = players[index] and lp:IsAlive() and clamp(alpha[index] + fade, 0, a)
				or clamp(alpha[index] - fade, 0, a)
		else
			alpha[index] = v:IsPlayer()
					and v:GetTeamNumber() ~= lp:GetTeamNumber()
					and v:IsAlive()
					and lp:IsAlive()
					and dist <= 1500
					and clamp(alpha[index] + fade, 0, a)
				or clamp(alpha[index] - fade, 0, a)
		end
	...
	for k, v in pairs(temp) do
		...
		for i = 1, 2, 0.2 do
			...
			if
				...
				or psx < 100
				or psy < 100
				or psx > screen_size_x + 500
				or psy > screen_size_y + 100
				or not OOV_always:GetValue()
			then
				circle_outline(
					screen_size_x,
					screen_size_y,
