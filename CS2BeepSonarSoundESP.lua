--[==[
Original was for v4 and made by ambien55 (https://aimware.net/forum/user/240129)
ForumThread: https://aimware.net/forum/thread/101870

2024-5-27 2878713023(https://aimware.net/forum/user/366789) Fixed for cs2 v5

]==]

    local REFER = gui.Reference "Misc"
    local BEEPTAB = gui.Tab(REFER, "sonar.esp.tab", "Sonar ESP")
    local GROUP = gui.Groupbox(BEEPTAB, "Sonar ESP", 16, 16, 297, 300)
    local ENABLE = gui.Checkbox(GROUP, "sonar.enabled", "Enable", false)
    local VOLUME = gui.Slider(GROUP, "sonar.volume", "Volume", 50, 0.5, 100, 0.5)
    local SOUND = gui.Combobox(GROUP, "sonar.sound", "Sound", "Buttons Bell 1", "Buttons Blip 1", "Buttons Blip 2", "Button 8", "Button 9")
    local DISTANCE = gui.Slider(GROUP, "sonar.distance", "Distance", 200, 100, 1500, 50)
    local FOV = gui.Slider(GROUP, "sonar.fov", "FOV", 10, 1, 180, 1)
    local FOV_ROTATION = gui.Slider(GROUP, "sonar.rotation", "FOV rotation", 360, 0, 360, 1)
    local DISTANCEBASED = gui.Checkbox(GROUP, "sonar.distancebasedfreq", "Distancebased Beep-frequency", false)
    local DISTANCEBASED_VOLUME = gui.Checkbox(GROUP, "sonar.distancebasedvol", "Distancebased Beep-volume", false)
    ...
    --- Variables
    local lastBeep = 0
    local sounds = {"buttons/bell1", "buttons/blip1", "buttons/blip2", "buttons/button8", "buttons/button9"}
        ...
        if pLocal and pLocal:IsAlive() and ENABLE:GetValue() then
            ...
            for i = 1, 64, 1 do
                ...
                if pEntity then
                    ...
                    if m_hPawn and m_hPawn:IsAlive() then
                        if m_hPawn:GetTeamNumber() ~= pLocal:GetTeamNumber() then
                            enemies = enemies + 1
                            local pEntityOrigin = m_hPawn:GetAbsOrigin()
            ...
            if enemiesInRange > 0 and lastBeep < globals.RealTime() - replayGap then
                ...
                local vol = VOLUME:GetValue() / 1000

                if DISTANCEBASED_VOLUME:GetValue() then
                    vol = (1 - (closestDistance / 1500)) * vol
                end

                client.SetConVar("snd_toolvolume", vol, true);
                client.Command("play sounds/" .. button, true)
    ...
    callbacks.Register("Draw", "hkDraw", hkDraw)
